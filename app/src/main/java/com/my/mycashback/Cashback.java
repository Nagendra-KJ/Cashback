...
public class Cashback extends AppCompatActivity {
    ...
    void submit() {
        if (validate()) {
            db.collection("Users").whereEqualTo("phoneNumber", phoneNumber.getText().toString()).
                    get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @RequiresApi(api = Build.VERSION_CODES.KITKAT)
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if (task.isSuccessful()) {
                        int size = Objects.requireNonNull(task.getResult()).getDocuments().size();
                        Log.e("Size", size + "");
                        if (size == 0) {
                            ...
                            Toast.makeText(Cashback.this, "Submitting to database", Toast.LENGTH_SHORT).show();
                            ...
                            AlertDialog.Builder builder = new AlertDialog.Builder(Cashback.this);
                            builder.setTitle("Error");
                            builder.setMessage("Cashback cannot be given twice");
                            builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    dialog.dismiss();
